  ...
  function isLoadingDone() {
    ...
    if(returnAapData().lists.length && returnAapData().members.length){
  ...
  function deleteMember(target) {
    ...
    for (let i = 0; i < returnAapData().members.length; i++){

      if (memberLiId === returnAapData().members[i].id){
        returnAapData().members.splice(i, 1);
        membersPage();
      }
    }
  ...
  function membersPage() {
    ...
    returnAapData().members.forEach((member) =>{
      ...
      //creating member delete button.
      const saveBtn = createElement('button',['btn','delete-card-style','saveBtn','fade-out'],memberLi);
  ...
  function createMember() {
    ...
    returnAapData().members.push(newMember);
  ...
  function showBtns(event) {
    ...
    if (target.textContent === 'Delete'){
      ...
      memberLi.style.border = '2px solid blue';

      $(memberLi).fadeOut(3000, function () {
  ...
  function addEventListenerNewMemberBtn() {
...
// -------------------Board----------------------
  ...
  function addingAList(newList) {
    ...
    if (newList === undefined) {
      newList = { id: uuid(), title: 'New List', tasks: [] };
      returnAapData().lists.push(newList);
      initPageByHash();
    }
        ...
        for (let task of obj.tasks) {
          ...
          function memberCreater() {
            ...
            for (let member of task.members) {
              ...
              let found = false;
              for (let m of returnAapData().members)
                if (m.name === member)
                  found = true;
    ...
    const listTemplate = `
        <div class="panel panel-default temp">  
          <div class="panel-heading panel-size">
            ...
            <span class="newList listNameStyle">`+ newList.title +`</span>
  ...
  function initPageByHash() {
    ...
    if (hash === '#board') {
      ...
      for(let list of returnAapData().lists) {
  ...
  function removeList(event) {
    ...
    for ( let list in returnAapData().lists ) {
      if (returnAapData().lists[list].id == listId) {
        confirm("are you sure you want to delete ["+ returnAapData().lists[list].title +"]?");
        returnAapData().lists.splice(list,1);
      }
    }
    initPageByHash();
  ...
  function createACard(listId) {
    ...
    for(let list in returnAapData().lists) {
      if (returnAapData().lists[list].id === listId) {
        returnAapData().lists[list].tasks.push(newCard);
        // newCard.scrollTop();
        initPageByHash();
      }
    }
  ...
  function openModal(event) {
    ...
    // find card
    for (const list of returnAapData().lists)
    ...
    for (const member of returnAapData().members) {
      let isMemberOnCard = '';
      ...
      for (let task in list.tasks) {
        ...
        if (list.tasks[task].id === cardId){
          ...
          if (list.title != toList) {
            for (let listx of returnAapData().lists) {
  ...
  function titleInputKeyHandler(event) {
    ...
    if (event.keyCode === 13) {
      ...
      for (let list in returnAapData().lists){
        if (returnAapData().lists[list].title == titleElm.innerHTML)
          returnAapData().lists[list].title = value;
      }
...
// -----ajax----------JSON--------
  ...
  function boardDataHendler(event) {
    ...
    returnAapData().lists = data.board;
  ...
  function membersDataHendler(event) {
    ...
    returnAapData().members = data.members;
